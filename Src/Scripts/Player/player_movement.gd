...
func _physics_process(delta):
	...
	# Apply friction
	player.velocity.x *= 1 / (1 + (delta * friction))
...
func is_feet_on_ground():
	if player.is_on_floor() and default_gravity >= 0:
		return true
	if player.is_on_ceiling() and default_gravity <= 0:
		return true
...
func apply_gravity_multipliers_to(gravity) -> float:
	if player.velocity.y * sign(default_gravity) > 0: # If we are falling
		gravity *= falling_gravity_multiplier
	
	# if we released jump and are still rising
	elif player.velocity.y * sign(default_gravity) < 0:
		if not holding_jump: 
